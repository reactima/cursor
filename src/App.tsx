...
const useCursorTracker = (
  ...
  useEffect(() => {
    ...
    const onMouse = (e: L.LeafletMouseEvent) => {
      ...
      report(msg) // optimistic local update
...
const App: React.FC = () => {
  ...
  const toggle = () => {
    ...
    setUserUUID(newUID)
    setCursors({})
    const socket = new ReconnectingWebSocket(`${WS_URL}?user_uuid=${newUID}&project_uuid=${projectUUID}`)
  ...
  // derived list of users (show own first)
  const users = useMemo(() => {
